...
class GStatus:
	...
	def draw(self, screen, status):
		...
		if status.gameProgress == GameProgress.OVER:
			resultS, resultBB = self.GAME_FONT.render("{} Wins!".format(playerName), "black", size=14)

			sf.blit(resultS, ((self.size[0] - resultBB.w) / 2, (self.size[1] - resultBB.h) / 2))
		else:
			turnS, turnBB = self.GAME_FONT.render("Turn: {}".format(playerName), "black", size=14)
			moveS, moveBB = self.GAME_FONT.render("Moves Left: {}".format(status.movesLeft), "black", size=14)
